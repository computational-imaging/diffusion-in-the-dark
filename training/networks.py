...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        in_channels = img_channels * 3 if fullres else img_channels * 2
        if self_norm:
        ...
        # print(self_norm, scale_norm, border_norm, fullres)
    ...
    def forward(self, x, raw_in, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        raw_in = raw_in.to(torch.float32)
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda') else torch.float32
        ...
        # print('check mean: ', (c_in * x).mean(), (c_in * x).std(), raw_in.mean(), raw_in.std())
        # F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)

        # print('hi')
        # breakpoint()
        F_x = self.model(torch.cat([(c_in * x).to(dtype), raw_in.detach().to(dtype)], dim=1), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
